<%@ Page Language="C#" AutoEventWireup="true" CodeFile="procpanel.aspx.cs" Inherits="proc_procpanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
    ...
    <script type="text/javascript">
        function getRecord() {
            ...
            procnum = getSpecificGridItem("grdresults", 0, "ProcNum");
            ...
            parent.mainmodule.document.location.href = "procmain.aspx?mode=edit&procnum=" + procnum;
            $("#hidpushrecord").val("0");
        }

        function rowClicked(sender, eventArgs) {
            var procnum = eventArgs.getDataKeyValue("ProcNum");
            var proceed = true;
            try {
    ...
    <div id="header-wrap">
        <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
            ...
            <table width="90%">
                <tr>
                    <td align="right">
                        <a href="javascript:setLogoffCookie('logoff','1','PROC')">Log Off</a>
                    </td>
                </tr>
            </table>
        </div>
                ...
                <tr>
                    <td width="40%">
                        <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
                            ...
                            color: #FFFFFF;" onclick="chgframesizewithvalue('PROC')"/>
                    </td>
                    <td width="30%">
                    </td>
...
<script type="text/javascript">
    ...
    function updatecounts(sender) {
        ...
        if ((mode == "queryresult" && sender.get_id() == "grdresults") || (mode == "worklist" && sender.get_id() == "grdworklist") || (mode == "recentlist" && sender.get_id() == "grdrecentlist")) {
            ...
            var procnum = "";
            if (parent.mainmodule.document.getElementById("txtprocnum") != null)
                procnum = parent.mainmodule.document.getElementById("txtprocnum").value;
            var index = -1;
            ...
            if (procnum != "") {
                for (var i = 0; i < allRows.length; i++) {
                    if (allRows[i].getDataKeyValue("ProcNum") == procnum) {
...
<script type="text/javascript">
    ...
    function RadGrid_RowSelected(sender, args) {
        var mailID = args.getDataKeyValue("procnum");
        if (!selected[mailID]) {
    ...
    function worklistadd(grid) {
        ...
        $(document).ready(function () {
            ...
            for (var i = 0; i < selectedRows.length; i++) {
                ...
                var cell = MasterTable.getCellByColumnUniqueName(row, "ProcNum");
                ...
                if (i == 0) {
                    ...
                    proclist = proclist + "^" + cellval;
                }
                procxml = procxml + "<IDS>" + cellval + "</IDS>";
            }
            ...
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServiceProc.svc/ProcAddtoWorkList",
                data: { "paramXML": procxml },
                dataType: "json",
    ...
    function removeworklist() {
        ...
        $(document).ready(function () {
            ...
            var procxml = "<?xml version='1.0' encoding='UTF-8'?><roots><Linkid>";
            ...
            procxml = procxml + "</Linkid>";
            procxml = procxml + "<LinkModule>Procedure</LinkModule>";
            procxml = procxml + "<LinkType>" + hidmode + "</LinkType>";
            procxml = procxml + "</roots>";

            $.ajax({
                type: "GET",
                url: "../InternalServices/ServiceProc.svc/ProcDelWorkList",
                data: { "procxml": procxml },
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                beforeSend: function (xhr, settings) {
                },
