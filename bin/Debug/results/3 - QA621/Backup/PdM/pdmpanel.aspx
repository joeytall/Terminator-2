<%@ Page Language="C#" AutoEventWireup="true" CodeFile="pdmpanel.aspx.cs" Inherits="pdm_pdmpanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
  ...
  <script type="text/javascript">
    ...
    function getRecord() {
        ...
        pdmnum = getSpecificGridItem("grdresults", 0, "PDMNum");
        ...
        parent.mainmodule.document.location.href = "pdmmain.aspx?mode=edit&pdmnum=" + pdmnum;
        $("#hidpushrecord").val("0");
    }

    function rowClicked(sender, eventArgs) {
      var pdmnum = eventArgs.getDataKeyValue("PDMNum");
      var proceed = true;
      try {
  ...
  <div id="header-wrap">
    <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
      ...
      <table width="90%">
        <tr>
          <td align="right">
            <a href="javascript:setLogoffCookie('logoff','1','PDM')">Log Off</a>
          </td>
        </tr>
      </table>
    </div>
        ...
        <tr>
          <td width="40%">
            <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
              ...
              color: #FFFFFF;" onclick="chgframesizewithvalue('PDM')"/>
          </td>
          <td width="30%">
          </td>
...
<script type="text/javascript">
  ...
  function updatecounts(sender) {
    ...
    if ((mode == "queryresult" && sender.get_id() == "grdresults") || (mode == "worklist" && sender.get_id() == "grdworklist") || (mode == "recentlist" && sender.get_id() == "grdrecentlist")) {
      ...
      var pdmnum = "";
      if (parent.mainmodule.document.getElementById("txtpdmnum") != null)
        pdmnum = parent.mainmodule.document.getElementById("txtpdmnum").value;
      var index = -1;
      ...
      if (pdmnum != "") {
        for (var i = 0; i < allRows.length; i++) {
          if (allRows[i].getDataKeyValue("PDMNum") == pdmnum) {
...
<script type="text/javascript">
  ...
  function RadGrid_RowSelected(sender, args) {
    var mailID = args.getDataKeyValue("pdmnum");
    //alert(mailID);
    if (!selected[mailID]) {
  ...
  function worklistadd(grid) {
    ...
    $(document).ready(function () {
      ...
      var xml = "<?xml version='1.0' encoding='UTF-8'?><pdms><pdm>";

      for (var i = 0; i < selectedRows.length; i++) {
        var row = selectedRows[i];
        var cell = MasterTable.getCellByColumnUniqueName(row, "PDMNum");
      ...
      $.ajax({
        type: "GET",
        url: "../InternalServices/ServicePDM.svc/AddtoWorkList",
        data: { "xml": xml },
        dataType: "json",
  ...
  function removeworklist() {
    ...
    $(document).ready(function () {
      ...
      xml = xml + "<LinkModule>PDM</LinkModule>";
      ...
      $.ajax({
        type: "GET",
        url: "../InternalServices/ServicePDM.svc/DelWorkList",
        data: { "xml": xml },
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        beforeSend: function (xhr, settings) {
