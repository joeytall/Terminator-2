<%@ Page Language="C#" AutoEventWireup="true" CodeFile="popanel.aspx.cs" Inherits="purchase_purchasepanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
    ...
    <script type="text/javascript">
        ...
        function getRecord() {
            ...
            ponum = getSpecificGridItem("grdresults", 0, "PoNum");
            ...
            parent.mainmodule.document.location.href = "pomain.aspx?mode=edit&ponum=" + ponum;
            $("#hidpushrecord").val("0");
        }
      ...
      /*
        ...
        */
        function rowClicked(sender, eventArgs) {
            var ponum = eventArgs.getDataKeyValue("PoNum");
            var proceed = true;
            try {
    ...
    <div id="header-wrap">
        <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
            ...
            <table width="90%">
                <tr>
                    <td align="right">
                        <a href="javascript:setLogoffCookie('logoff','1','PO')">Log Off</a>
                    </td>
                </tr>
            </table>
        </div>
                ...
                <tr>
                    <td width="40%">
                        <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
                            ...
                            color: #FFFFFF;" onclick="chgframesizewithvalue('PO')"/>
                    </td>
...
<script type="text/javascript">
    ...
    function updatecounts(sender) {
        ...
        /*
            ...
            */
            var ponum = "";
            if (parent.mainmodule.document.getElementById("txtponum") != null)
                ponum = parent.mainmodule.document.getElementById("txtponum").value;
            ...
            if (ponum != "") {
                for (var i = 0; i < allRows.length; i++) {
                    if (allRows[i].getDataKeyValue("PoNum") == ponum) {
...
<script type="text/javascript">
    ...
    function worklistadd(grid) {
        $(document).ready(function () {
            ...
            var venderlist = "";
            var paramxml = "<?xml version='1.0' encoding='UTF-8'?><root><purchase>";

            for (var i = 0; i < selectedRows.length; i++) {
                var row = selectedRows[i];
                var cell = MasterTable.getCellByColumnUniqueName(row, "PoNum");
                var cellval = cell.innerHTML;
                paramxml = paramxml + "<ponum>" + cellval + "</ponum>";
            }

            paramxml = paramxml + "</purchase></root>";
            //alert(paramxml);
            //alert("1");
            ...
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServicePO.svc/AddtoWorklist",
    ...
    function removeworklist() {
        $(document).ready(function () {
            ...
            var xmlparam = "<?xml version='1.0' encoding='UTF-8'?><roots><purchase>";

            for (i = 0; i < selectedRows.length; i++) {
                var row = selectedRows[i];
                var cell = MasterTable.getCellByColumnUniqueName(row, "PoNum");
                var cellval = cell.innerHTML;
                xmlparam = xmlparam + "<ponum>" + cellval + "</ponum>";
            }

            xmlparam = xmlparam + "</purchase></roots>";
            alert(xmlparam);
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServicePO.svc/RemovefromWorklist",
