<%@ Page Language="C#" AutoEventWireup="true" CodeFile="routepanel.aspx.cs" Inherits="route_routepanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
  ...
  <script type="text/javascript">
      function getRecord() {
          ...
          routename = getSpecificGridItem("grdresults", 0, "RouteName");
          ...
          parent.mainmodule.document.location.href = "routemain.aspx?mode=edit&routename=" + routename;
          $("#hidpushrecord").val("0");
      }

      function rowClicked(sender, eventArgs) {
      var routename = eventArgs.getDataKeyValue("RouteName");
      var proceed = true;
      try {
  ...
  <div id="header-wrap">
    <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
      ...
      <table width="90%">
        <tr>
          <td align="right">
            <a href="javascript:setLogoffCookie('logoff','1','ROUTE')">Log Off</a>
          </td>
        </tr>
      </table>
    </div>
        ...
        <tr>
          <td width="40%">
            <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
              ...
              color: #FFFFFF;" onclick="chgframesizewithvalue('ROUTE')"/>
          </td>
          <td width="30%">
          </td>
  ...
  <asp:Panel ID="MainControlsPanel3" runat="server" CssClass="PanelControlsPanelClass">
    <asp:SqlDataSource ID="RecentListSqlDataSource" runat="server" ProviderName="System.Data.OleDb">
    </asp:SqlDataSource>
  </asp:Panel>
...
<script type="text/javascript">
  ...
  function updatecounts(sender) {
    ...
    if ((mode == "queryresult" && sender.get_id() == "grdresults") || (mode == "worklist" && sender.get_id() == "grdworklist") || (mode == "recentlist" && sender.get_id() == "grdrecentlist")) {
      ...
      var routename = "";
      if (parent.mainmodule.document.getElementById("txtroutename") != null)
        routename = parent.mainmodule.document.getElementById("txtroutename").value;
      var index = -1;
      ...
      if (routename != "") {
        for (var i = 0; i < allRows.length; i++) {
          if (allRows[i].getDataKeyValue("RouteName") == routename) {
...
<script type="text/javascript">
  ...
  function RadGrid_RowSelected(sender, args) {
    var mailID = args.getDataKeyValue("RouteName");
    //alert(mailID);
    if (!selected[mailID]) {
  ...
  function toolbarclicked(sender, args) {
    ...
    switch (commandname) {
      case "addworklist":
        if (addToWorklist()) {
          var hidmode = $('input:hidden[name=hidMode]').val(); //queryresult  recentlist
          var grid;
          ...
          //__doPostBack("toolbar", '{\"Command\" : \"Select\"}');
          //__doPostBack("addworklist", "");
  ...
  function worklistadd(grid) {
    ...
    $(document).ready(function () {
      ...
      var xml = "<?xml version='1.0' encoding='UTF-8'?><templates><template>";

      for (var i = 0; i < selectedRows.length; i++) {
        var row = selectedRows[i];
        var cell = MasterTable.getCellByColumnUniqueName(row, "RouteName");
        ...
        xml = xml + "<IDS>" + cellval + "</IDS>";
      }

      xml = xml + "</template></templates>";

      $.ajax({
        type: "GET",
        url: "../InternalServices/ServiceRoute.svc/AddtoWorkList",
        data: { "xml": xml },
        dataType: "json",
  ...
  function removeworklist() {
    ...
    $(document).ready(function () {
      ...
      var xml = "<?xml version='1.0' encoding='UTF-8'?><roots><Linkid>";
      ...
      xml = xml + "<LinkModule>Route</LinkModule>";
      xml = xml + "<LinkType>" + hidmode + "</LinkType>";

      xml = xml + "</roots>";

      $.ajax({
        type: "GET",
        url: "../InternalServices/ServiceRoute.svc/DelWorkList",
        data: { "xml": xml },
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        beforeSend: function (xhr, settings) {
  ...
  function getcurrentselectedpms() {
    ...
    if (grid != null) {
      ...
      if (selectedrows.length > 0) {
        ...
        for (i = 0; i < selectedrows.length; i++) {
          var row = selectedrows[i];
          var cell = masterview.getCellByColumnUniqueName(row, "PDMNum");
          var pdmnum = cell.innerHTML;
          if (selectedpms == "")
            selectedpms = pdmnum;
          else
            selectedpms = selectedpms + "," + pdmnum;
        }
      }
      else {
        return "";
