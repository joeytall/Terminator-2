<%@ Page Language="C#" AutoEventWireup="true" CodeFile="pmpanel.aspx.cs" Inherits="pm_pmpanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
    ...
    <script type="text/javascript">
        ...
        function getRecord() {
            ...
            pmnum = getSpecificGridItem("grdresults", 0, "PmNum");
            ...
            parent.mainmodule.document.location.href = "pmmain.aspx?mode=edit&pmnum=" + pmnum;
            $("#hidpushrecord").val("0");
        }
    ...
    <div id="header-wrap">
        <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
            ...
            <table width="90%">
                <tr>
                    <td align="right">
                        <a href="javascript:setLogoffCookie('logoff','1','PM')">Log Off</a>
                    </td>
                </tr>
            </table>
        </div>
                ...
                <tr>
                    <td width="40%">
                        <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
                            ...
                            color: #FFFFFF;" onclick="chgframesizewithvalue('PM')"/>
                    </td>
                    <td width="30%">
                    </td>
...
<script type="text/javascript">
    ...
    function updatecounts(sender) {
        ...
        if ((mode == "queryresult" && sender.get_id() == "grdresults") || (mode == "worklist" && sender.get_id() == "grdworklist") || (mode == "recentlist" && sender.get_id() == "grdrecentlist")) {
            ...
            var pmnum = "";
            if (parent.mainmodule.document.getElementById("txtpmnum") != null)
                pmnum = parent.mainmodule.document.getElementById("txtpmnum").value;
            var index = -1;
            ...
            if (pmnum != "") {
                for (var i = 0; i < allRows.length; i++) {
                    if (allRows[i].getDataKeyValue("PmNum") == pmnum) {
...
<script type="text/javascript">
    ...
    function RadGrid_RowSelected(sender, args) {
        var mailID = args.getDataKeyValue("pmnum");
        //alert(mailID);
        if (!selected[mailID]) {
    ...
    function specsearch() {
        var URL;
        URL = "../util/specsearch.aspx?module=pmnum";
    ...
    function worklistadd(grid) {
        ...
        $(document).ready(function () {
            ...
            var xml = "<?xml version='1.0' encoding='UTF-8'?><pms><pm>";

            for (var i = 0; i < selectedRows.length; i++) {
                var row = selectedRows[i];
                var cell = MasterTable.getCellByColumnUniqueName(row, "PmNum");
            ...
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServicePM.svc/AddtoWorkList",
                data: { "xml": xml },
                dataType: "json",
    ...
    function removeworklist() {
        ...
        $(document).ready(function () {
            ...
            xml = xml + "<LinkModule>PM</LinkModule>";
            ...
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServicePM.svc/DelWorkList",
                data: { "xml": xml },
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                beforeSend: function (xhr, settings) {
        ...
        if (grid != null) {
            ...
            if (selectedrows.length > 0) {
                ...
                for (i = 0; i < selectedrows.length; i++) {
                    var row = selectedrows[i];
                    var cell = masterview.getCellByColumnUniqueName(row, "PmNum");
                    var pmnum = cell.innerHTML;
                    if (selectedpms == "")
                        selectedpms = pmnum;
                    else
                        selectedpms = selectedpms + "," + pmnum;
                }
            }
            else {
                return "";
