<%@ Page Language="C#" AutoEventWireup="true" CodeFile="locationpanel.aspx.cs" Inherits="location_locationpanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
    ...
    <script type="text/javascript">
        function getRecord() {
            ...
            location = getSpecificGridItem("grdresults", 0, "Location");
            ...
            parent.mainmodule.document.location.href = "locationmain.aspx?mode=edit&location=" + location;
            $("#hidpushrecord").val("0");
        }

        function rowClicked(sender, eventArgs) {
            var location = eventArgs.getDataKeyValue("Location");
            var proceed = true;
            try {
    ...
    <div id="header-wrap">
        <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
            ...
            <table width="90%">
                <tr>
                    <td align="right">
                        <a href="javascript:setLogoffCookie('logoff','1','LOC')">Log Off</a>
                    </td>
                </tr>
            </table>
        </div>
                ...
                <tr>
                    <td width="40%">
                        <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
                            ...
                            color: #FFFFFF;"  onclick="chgframesizewithvalue('LOC')"/>
                    </td>
...
<script type="text/javascript">
    ...
    function updatecounts(sender) {
        ...
        if ((mode == "queryresult" && sender.get_id() == "grdresults") || (mode == "worklist" && sender.get_id() == "grdworklist") || (mode == "recentlist" && sender.get_id() == "grdrecentlist")) {
            ...
            var location = "";
            if (parent.mainmodule.document.getElementById("txtlocation") != null)
                location = parent.mainmodule.document.getElementById("txtlocation").value;
            ...
            if (location != "") {          
                for (var i = 0; i < allRows.length; i++) {                    
                    if (allRows[i].getDataKeyValue("Location") == location) {                        
...
<script type="text/javascript">
  ...
  $(window).load(function () {
    ...
    function RadGrid_RowSelected(sender, args) {
        var mailID = args.getDataKeyValue("location");
        if (!selected[mailID]) {
    ...
    function specsearch() {
        var URL;
        URL = "../util/specsearch.aspx?module=location";
...
<script type="text/javascript">
    ...
    function worklistadd(grid) {
        ...
        $(document).ready(function () {
            ...
            var paramxml = "<?xml version='1.0' encoding='UTF-8'?><root><location>";

            for (var i = 0; i < selectedRows.length; i++) {
                var row = selectedRows[i];
                var cell = MasterTable.getCellByColumnUniqueName(row, "Location");
            ...
            paramxml = paramxml + "<LinkModule>location</LinkModule>";
            paramxml = paramxml + "</location></root>";
            if (!allowsubmit()) return;
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServiceLoc.svc/AddtoWorkList",
    ...
    function OnSuccessadd(grid) {
        ...
        for (var i = 0; i < selectedRows.length; i++) {
            //var firstDataItem = grid.get_masterTableView().get_selectedItems()[i];//.get_dataItems()[i];
    ...
    function removeworklist() {
        ...
        $(document).ready(function () {
            ...
            xmlparam = xmlparam + "<LinkModule>location</LinkModule>";
            ...
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServiceLoc.svc/DelWorkList",
