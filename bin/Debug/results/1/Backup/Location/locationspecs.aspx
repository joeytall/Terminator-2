<%@ Page Language="C#" AutoEventWireup="true" CodeFile="locationspecs.aspx.cs" Inherits="location_locationspecs" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
    ...
    <asp:Panel ID="MainControlsPanel" runat="server"  CssClass="MainControlsPanelClass">
        ...
        <telerik:RadAjaxManager ID="ajaxmanager" runat="server">
            <AjaxSettings>
                <telerik:AjaxSetting AjaxControlID="grdspecs">
                    <UpdatedControls>
                        <telerik:AjaxUpdatedControl ControlID="grdspecs" />
                    </UpdatedControls>
                </telerik:AjaxSetting>
            </AjaxSettings>
            <ClientEvents OnRequestStart="RequestStart" OnResponseEnd="ResponseEnd" />
        </telerik:RadAjaxManager>
        <asp:SqlDataSource ID="SpecsSqlDataSource" runat="server" ProviderName="System.Data.OleDb">
        </asp:SqlDataSource>
        <asp:HiddenField ID="hidSpecs" runat="server" />
        <asp:ValidationSummary ID="ValidationSummary1" runat="server" ShowMessageBox="True" ValidationGroup="specs"
            ShowSummary="False" />
    </asp:Panel>
...
<script type="text/javascript">
  ...
  function refreshGrid() {
    var grid = $find("<%=grdspecs.ClientID%>");
    //if (grid!=null)
