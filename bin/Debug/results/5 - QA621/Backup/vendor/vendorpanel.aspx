<%@ Page Language="C#" AutoEventWireup="true" CodeFile="vendorpanel.aspx.cs" Inherits="vendor_vendorpanel" %>
...
<body style="background-image: url(../IMAGES/back.png); background-repeat: repeat-x;
    margin: 0px; padding: 0px;" onunload="chgframesize('VENDOR')">
    ...
    <script type="text/javascript">
        ...
        function getRecord() {
            ...
            vendor = getSpecificGridItem("grdresults", 0, "CompanyCode");
            ...
            parent.mainmodule.document.location.href = "vendormain.aspx?mode=edit&vendor=" + vendor;
            $("#hidpushrecord").val("0");
        }

        function rowClicked(sender, eventArgs) {
            var vendor = eventArgs.getDataKeyValue("CompanyCode");
            var proceed = true;
            try {
    ...
    <div id="header-wrap">
        <div style="display: block; width: 100%; height: 30px; position: relative; padding: 0 0 0 10px;
            ...
            <table width="90%">
                <tr>
                    <td align="right">
                        <a href="javascript:setLogoffCookie('logoff','1','VENDOR')">Log Off</a>
                    </td>
                </tr>
            </table>
        </div>
                ...
                <tr>
                    <td width="40%">
                        <asp:Label ID="lblListType" runat="server" Text="" Style="position: absolute; top: 9px;
                            ...
                            color: #FFFFFF;" onclick="chgframesizewithvalue('VENDOR')"/>
                    </td>
...
<script type="text/javascript">
    ...
    function updatecounts(sender) {
        ...
        /*
            ...
            */
            var vendor = "";
            if (parent.mainmodule.document.getElementById("txtcompanycode") != null)
                vendor = parent.mainmodule.document.getElementById("txtcompanycode").value;
            ...
            if (location != "") {
                for (var i = 0; i < allRows.length; i++) {
                    if (allRows[i].getDataKeyValue("CompanyCode") == vendor) {
...
<script type="text/javascript">
    ...
    function RadGrid_RowSelected(sender, args) {
        var mailID = args.getDataKeyValue("CompanyCode");
        if (!selected[mailID]) {
    ...
    function specsearch() {
        var URL;
        URL = "../util/specsearch.aspx?module=vendor";
...
<script type="text/javascript">
    ...
    function worklistadd(grid) {
        $(document).ready(function () {
            ...
            var venderlist = "";
            var paramxml = "<?xml version='1.0' encoding='UTF-8'?><root><vendors>";

            for (var i = 0; i < selectedRows.length; i++) {
                var row = selectedRows[i];
                var cell = MasterTable.getCellByColumnUniqueName(row, "CompanyCode");
                var cellval = cell.innerHTML;
                paramxml = paramxml + "<vendor>" + cellval + "</vendor>";
            }

            paramxml = paramxml + "</vendors></root>";

        //    alert(paramxml);     // alert

            if (!allowsubmit()) return;

            $.ajax({
                type: "GET",
                url: "../InternalServices/ServiceVendor.svc/AddtoWorklist",
    ...
    function removeworklist() {
        $(document).ready(function () {
            ...
            var xmlparam = "<?xml version='1.0' encoding='UTF-8'?><roots><vendors>";

            for (i = 0; i < selectedRows.length; i++) {
                var row = selectedRows[i];
                var cell = MasterTable.getCellByColumnUniqueName(row, "CompanyCode");
                var cellval = cell.innerHTML;
                xmlparam = xmlparam + "<vendor>" + cellval + "</vendor>";
            }

            xmlparam = xmlparam + "</vendors></roots>";
            alert(xmlparam);
            $.ajax({
                type: "GET",
                url: "../InternalServices/ServiceVendor.svc/DelfromWorklist",
